def validate_folder(kwargs):
    ...
    for key, value in keys:
        assert bool(kwargs.get(key)) in value, \
            f'Validation failed, {key} must be set'
...
def validate_file(kwargs):
    ...
    for key, value in keys:
        if key == 'size':
            assert kwargs.get(key, 0) > 0, \
                'Validation failed, size must be positive'

        elif key == 'type':
            assert kwargs.get(key) in value, \
                f"Awaited type in ('folder', 'file'), got {kwargs.get(key)}"

        else:
            assert bool(kwargs.get(key, 0)) in value, \
                f'Validation failed, {key} must be set'
